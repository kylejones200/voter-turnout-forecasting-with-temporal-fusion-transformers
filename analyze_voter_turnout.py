...
def analyze_turnout_patterns(df):
    ...
    presidential_bump = (
        presidential_elections["Turnout_Rate"].mean()
    ...
    t_stat, p_value = stats.ttest_ind(
        presidential_elections["Turnout_Rate"], midterm_elections["Turnout_Rate"]
    )
...
def estimate_trend(df):
    ...
    if slope > 0:
        logger.info(
            f"  Interpretation: Turnout increasing by {slope:.4f}% per year on average"
        )
    else:
        logger.info(
            f"  Interpretation: Turnout decreasing by {abs(slope):.4f}% per year on average"
        )
...
def calculate_presidential_effect(df, trend):
    ...
    avg_presidential_effect = np.mean(presidential_detrended) - np.mean(
        midterm_detrended
    )
...
def visualize_time_series(
    df, trend, images_dir, year_range_str=None, plot: bool = False
):
    ...
    if plot:
        fig, ax = plt.subplots(
            figsize=tuple(config.get("output", {}).get("figsize", [12, 6]))
        )

        # Plot lines for presidential and midterm elections
        ax.plot(
            pres_data["Year"],
            pres_data["Turnout_Rate"],
            linewidth=1.5,
            color="black",
            label="Presidential",
            linestyle="-",
        ...
        plt.tight_layout()
        plt.savefig(
            images_dir / "voter_turnout_time_series.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        logger.info(
            f"Saved main time series to '{images_dir / 'voter_turnout_time_series.png'}'"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            images_dir / "voter_turnout_trend.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            images_dir / "voter_turnout_detrended.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def perform_statistical_decomposition(df, images_dir, plot: bool = False):
    ...
    try:
        decomposition = seasonal_decompose(
            ts_full, model="additive", period=2, extrapolate_trend="freq"
        )
        ...
        if plot:
            ...
            axes[0].set_title(
                f"Statistical Decomposition of Voter Turnout ({year_range_str})",
                fontsize=13,
                pad=10,
            )
            ...
            plt.tight_layout()
            plt.savefig(
                images_dir / "statistical_decomposition.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        logger.info(
            f"Saved statistical decomposition to '{images_dir / 'statistical_decomposition.png'}'"
        )
...
def forecast_turnout(
    df, trend_model, n_years_ahead=10, images_dir=None, plot: bool = False
):
    ...
    if plot:
        ...
        ax.plot(
            ...
            linewidth=1.5,
            color="gray",
            label="Midterm (Historical)",
            linestyle="--",
        )

        # Forecast
        ax.plot(future_years, y_forecast, linewidth=1.5, color="black", linestyle=":")
        ax.fill_between(
            future_years,
            lower_bound,
            upper_bound,
        ...
        if images_dir:
            plt.savefig(
                images_dir / "turnout_forecast.png", dpi=300, bbox_inches="tight"
            )
...
def create_summary_statistics(
    df, pres_elections, midterm_elections, presidential_bump, trend_slope, r_squared
):
    ...
    summary = {
        ...
        "highest_turnout": df.loc[
            df["Turnout_Rate"].idxmax(), ["Year", "Turnout_Rate", "Election_Type"]
        ].to_dict(),
        "lowest_turnout": df.loc[
            df["Turnout_Rate"].idxmin(), ["Year", "Turnout_Rate", "Election_Type"]
        ].to_dict(),
    }
...
def main():
    ...
    with open(summary_path, "w") as f:
        ...
        f.write("=" * 60 + "\n\n")
        f.write(
            f"Data Range: {summary['year_range'][0]} - {summary['year_range'][1]}\n"
        )
        ...
        f.write(
            f"Highest Turnout: {summary['highest_turnout']['Year']} "
            f"({summary['highest_turnout']['Turnout_Rate']:.1f}%, "
            f"{summary['highest_turnout']['Election_Type']})\n"
        )
        f.write(
            f"Lowest Turnout: {summary['lowest_turnout']['Year']} "
